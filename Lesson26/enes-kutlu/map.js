...
console.log('Ex.2');
const numberSquare = numbersExercise2.map((num) => num * num);
...
console.log('Ex.5');
const studentsWithGrades = studentsExercise5.map(person => `${person.name} ${person.grade}`)
...
console.log('Ex.7');
const splitedSenctences = sentencesExercise7.map(sentence => sentence.split(' '))
console.log(splitedSenctences);
...
console.log('Ex.8');
const usersEmail = usersExercise8.map(user => user.email)
...
console.log('Ex.9');
const booksWithAuthors = booksExercise9.map(book => `${book.title} - ${book.author}`);
...
const pricesCurrency = pricesExercise10.map(price => `€${price}`);
console.log(pricesCurrency);