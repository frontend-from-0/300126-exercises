...
const successBoard = document.getElementById("success")
const successBtn = document.querySelector(".success-gb")
...
function validateCvv(value) {
  const trimmedValue = value.trim();
  if (trimmedValue.length < 1) {
    cardCvvError.textContent = "This value is required";
    cardCvvError.classList.remove("hidden");
    formValid = false;
  } else if (!cardCvvPattern.test(trimmedValue)) {
    cardCvvError.textContent = "CVV should match the following format: 123";
    cardCvvError.classList.remove("hidden");
    formValid = false;
  } else {
...
form.addEventListener("submit", function (event) {
  ...
  if (formValid) {
    ...
    successBoard.classList.add("show");
  }
