...
const wordsConvertedUpperCase = wordsExercise1.map((word) =>
  word.toUpperCase(),
);
console.log("Ex1");
...
const numbersSquare = numbersExercise2.map((num) => num * num);
...
const lengtOfWords = wordsExercise4.map((words) => words.length);
console.log("Ex4");
console.log(lengtOfWords);
...
const studentWithGrades = studentsExercise5.map(
  (person) => `${person.name} ${person.grade}`,
);
...
const numberToString = numbersExercise6.map((num) => String(num));
console.log("Ex6");
...
const splitSentence = sentencesExercise7.map((sentenceWord) =>
  sentenceWord.split(" "),
);
...
const bookInformation = booksExercise9.map(
  (books) => `${books.author} ${books.title}`,
);
...
const priceWithSymbol = pricesExercise10.map((price) => `$${price}`);
