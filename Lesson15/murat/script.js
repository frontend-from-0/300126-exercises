...
function censorWord(sentence, target){
  if (typeof sentence !== "string" || typeof target !== "string"){
      console.log("Both 'sentence' and 'target' must be string values.")
      return;
    } 

    const newSentence = sentence.replaceAll(target, "****")
...
function calculate(a, operator, b) {
  if (typeof a !== "number" || typeof operator !== "string" || typeof b !== "number"){
      console.log("Invalid input types. 'a' and 'b' must be numbers, 'operator' must be a string.")
      return;
    } 
  switch(operator){
    case "+":
