...
fetch('https://jsonplaceholder.typicode.com/posts')
  .then((response) => response.json())
  .then((posts) => {
    // Sadece ilk 10 gönderiyi çekelim ki sayfa çok uzun olmasın (isteğe bağlı)
    ...
    limitedPosts.forEach((post) => {
      ...
      // 2. GÖRSEL EKLEME KISMI: img elementi oluştur
      const image = document.createElement('img');
      // Her gönderi için benzersiz bir random görsel URL'si
      image.src = `https://picsum.photos/400/200?random=${post.id}`;
      image.alt = post.title; // Erişilebilirlik için alt metin

      // 3. İçerik konteynerini oluştur (metinler için)
      const content = document.createElement('div');
      content.classList.add('post-card-content');

      // 4. Başlık ve içerik elementlerini oluştur
      ...
      // 5. Metinleri içerik konteynerine ekle
      content.appendChild(title);
      content.appendChild(body);

      // 6. ÖNCE görseli, SONRA metin içeriklerini karta ekle
      card.appendChild(image);
      card.appendChild(content);

      // 7. Kartı ana kapsayıcıya ekle
