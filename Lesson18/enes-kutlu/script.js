...
class ShoppingCart {
  ...
  getTotal() {
    ...
    if (this.#items.length > 0) {
      ...
      if (this.#discountedTotal !== null) {
        console.log(`Total amount is: ${this.#discountedTotal}`);
  ...
  applyDiscount(discountCode) {
    ...
    console.log("------- Discount applying -------");
    let totalDiscount = 0;
    let isFound = false;

    for (const values of this.#discount) {
      if (discountCode === values.name) {
        isFound = true;
        totalDiscount = this.getTotal() * ((100 - values.amount) / 100);
      }
    }
    ...
    if (!isFound) {
      ...
      this.#discountedTotal = totalDiscount;
      console.log(`Discount applied! Your new basket total: ${totalDiscount}`);
    }
    console.log("-------------");
  }
}
...
cart.applyDiscount("CAKE3");
cart.applyDiscount("CAKE30");
