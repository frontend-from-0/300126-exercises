...
function removeContact(name, contactList) {
  const index = contactList.findIndex((c) => c.name.toLowerCase() === name.toLowerCase());
...
*/
addContact('Teoman', '00905554443322', 'teoman@example.com', generalContacts);
viewContact('Teoman', generalContacts);
updateContact('Teoman', '00905559998877', 'teoman.celik@example.com', generalContacts);
...
// 1. Partial Name Search
function searchPartialName(query, contactList) {
  return contactList.filter((c) => c.name.toLowerCase().includes(query.toLowerCase()));
}
...
// 3. Search by multiple fields
function searchByMultiple(query, contactList) {
