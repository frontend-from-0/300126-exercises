...
fetchBtn.addEventListener('click', () => {
  ...
  fetch('https://dummyjson.com/users')
    ...
    .then((data) => {
      ...
      // Kendi eklediklerini ve API'den gelenleri tek bir listede birleştir
      // customUsers'ı başa yazdık ki senin eklediklerin listenin en üstünde görünsün
      const allUsers = [...customUsers, ...data.users];

      allUsers.map((user) => createUserCard(user));
    })
    .catch((error) => {
...
function createUserCard(user) {
  ...
  updateBtn.style.marginRight = 'var(--space-sm)';
...
function deleteUser(userId, userCardElement) {
  statusMessage.classList.add('hidden');

  // --- EĞER KULLANICI SENİN EKLEDİĞİN BİRİYSE HAFIZADAN SİL ---
  if (String(userId).startsWith('custom-')) {
    let storedUsers = JSON.parse(localStorage.getItem('customUsers')) || [];
    storedUsers = storedUsers.filter(u => u.id !== userId); // Sildiğimizi listeden çıkar
    localStorage.setItem('customUsers', JSON.stringify(storedUsers));
    
    if (userCardElement) userCardElement.remove();
    console.log(`Custom user ${userId} deleted successfully.`);
    return; // İşlem bitti, aşağıdaki API'ye gitmesini engelle
  }

  // --- NORMAL API'DEN SİLME ---
